...
class TestVIXIndicator:
    ...
    def test_vix_indicator_custom_parameters(self):
        ...
        vix = VIXIndicator(period=21, panic_threshold=25, volume_threshold=2.0, enabled=False)
    ...
    def test_vix_calculation_accuracy(self):
        ...
        data = {
            'close': prices,
            'volume': np.random.randint(1000, 3000, len(prices))
        }
        candles = pd.DataFrame(data)
    ...
    def test_vix_single_period_calculation(self):
        ...
        data = {
            'close': [100, 102, 98, 101, 99, 103, 97, 104, 96, 105, 95, 106, 94, 107, 93],
            'volume': [1000] * 15
        }
        candles = pd.DataFrame(data)
