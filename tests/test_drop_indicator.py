...
class TestSuddenPriceDropIndicator:
    ...
    def test_drop_indicator_initialization(self):
        ...
        assert (
                drop._SuddenPriceDropIndicator__drop_percentage == 0.05
    ...
    def test_drop_indicator_custom_parameters(self):
        ...
        drop = SuddenPriceDropIndicator(
            drop_percentage=10, lookback_period=10, volume_threshold=2.0, enabled=False
        )

        assert drop.is_enabled is False
        assert (
                drop._SuddenPriceDropIndicator__drop_percentage == 0.10
        )  # 10% converted to decimal
    ...
    def test_drop_detection_with_volume(self):
        ...
        drop = SuddenPriceDropIndicator(
            drop_percentage=8, volume_threshold=2.0, lookback_period=5
        )
    ...
    def test_no_drop_detection(self):
        ...
        data = {
            "close": [
                ...
                103,
                98,
            ],  # Small fluctuations, no significant drop
    ...
    def test_drop_threshold_exact(self):
        ...
        drop = SuddenPriceDropIndicator(
            drop_percentage=5, volume_threshold=1.5, lookback_period=5
        )
        drop.calculate(candles)
    ...
    def test_drop_gradual_decline(self):
        ...
        drop = SuddenPriceDropIndicator(
            drop_percentage=7, volume_threshold=1.5, lookback_period=5
        )
        drop.calculate(candles)
