...
class TestRSIIndicator:
    ...
    def test_rsi_indicator_custom_parameters(self):
        ...
        rsi = RSIIndicator(period=21, buy_threshold=25, sell_threshold=75, enabled=False)
    ...
    def test_rsi_calculation_accuracy(self):
        ...
        data = {
            'close': [44, 44.34, 44.09, 44.15, 43.61, 44.33, 44.83, 45.85, 47.37, 47.20, 46.57, 46.03, 46.98, 46.40, 46.12, 46.89]
        }
        candles = pd.DataFrame(data)
    ...
    def test_rsi_values_range(self, volatile_candles):
        ...
        if rsi_values:
            for value in rsi_values:
                assert 0 <= value <= 100, f"RSI value {value} is out of valid range [0, 100]"